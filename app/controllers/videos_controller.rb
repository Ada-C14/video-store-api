class VideosController < ApplicationController
  ...
  def show
    ...
    render json: video.as_json(only: [:id, :title, :release_date, :available_inventory]), status: :ok
  end
  ...
  def create
    ...
    if video.save
      render json: video.as_json(only: [:id, :title, :release_date, :available_inventory]), status: :created
      return
    else
      render json: {
          ok: false,
